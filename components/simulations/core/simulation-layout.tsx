...
export function SimulationLayout({
    ...
    return (
        <div className="flex flex-col min-h-[100dvh] lg:h-[100dvh] bg-background text-foreground font-[family-name:var(--font-sans)] lg:overflow-hidden">

            {/* ────────── HEADER ────────── */}
            {!isFullscreen && (
                <header className="relative z-50 flex items-center px-3 sm:px-5 py-3 border-b-[3px] border-black bg-background shrink-0">
                    <div className="flex items-center gap-3 w-full">
                        ...
                        <div className="flex items-center gap-2 overflow-hidden">
                            <div
                                className="w-3 h-8 sm:h-9 rounded-sm shrink-0 border-[2px] border-black"
                                style={{ backgroundColor: color }}
                            />
                            <h1 className="text-lg sm:text-xl font-black text-foreground uppercase tracking-tight truncate">
                                {title}
                            </h1>
                        </div>
                    </div>
            ...
            <main className="flex-1 flex flex-col lg:flex-row relative lg:overflow-hidden">
                ...
                <section className={cn(
                    ...
                    {/* The Canvas Frame */}
                    <div className={cn(
                        "relative w-full h-full overflow-hidden flex items-center justify-center transition-all",
                        isFullscreen
                            ? "rounded-none border-none bg-black"
                            : "rounded-xl border-[3px] border-black shadow-[4px_4px_0px_0px_#000] bg-[#1a1a1a]"
                    )}>
                        {children}
                        ...
                        <button
                            ...
                            className="absolute z-50 top-3 right-3 w-9 h-9 sm:w-10 sm:h-10 bg-white text-black border-[2px] border-black shadow-[2px_2px_0px_0px_#000] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] active:translate-x-[2px] active:translate-y-[2px] active:shadow-none transition-all flex items-center justify-center rounded-md"
                        >
                ...
                {/* ────────── INTERACTION PANEL ────────── */}
                {!isFullscreen && (
                    <aside className="flex-1 lg:h-full lg:w-[400px] xl:w-[420px] flex flex-col bg-background border-t-[3px] lg:border-t-0 lg:border-l-[3px] border-black z-40 relative shrink-0 pb-20 lg:pb-0">
                        {/* Tab Buttons */}
                        <div className="flex items-center gap-1 p-1.5 sm:p-2 shrink-0 border-b-[3px] border-black bg-background">
                            <TabButton
    ...
    return (
        <button
            ...
            className={cn(
                ...
                isActive
                    ? "text-black border-black shadow-[3px_3px_0px_0px_#000] -translate-x-[1px] -translate-y-[1px]"
                    : "border-transparent text-zinc-500 hover:text-foreground hover:bg-foreground/5"
            )}
...
function Placeholder({ text }: { text: string }) {
    return (
        <div className="h-full flex flex-col items-center justify-center text-zinc-600 space-y-4 py-12">
            <div className="w-14 h-14 border-[3px] border-dashed border-zinc-600 bg-zinc-800/50 flex items-center justify-center rounded-lg" />

...
export function FeedSidebar({ suggestedUsers = [] }: { suggestedUsers?: SidebarUser[] }) {
    ...
    return (
        <div className="space-y-6 lg:sticky lg:top-24">
            ...
            <div className="bg-[#EAB308] border-2 border-black rounded-[8px] p-6 shadow-[4px_4px_0px_0px_#000] relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 mb-3">
                        <div className="bg-black p-2 rounded-[8px] text-[#EAB308] border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,0.35)]">
                            <Sparkles className="w-5 h-5 fill-current" />
                        </div>
                        <h3 className="font-black text-xl text-black uppercase">Bilime Katkı Sağla</h3>
                    </div>
                    ...
                    <ViewTransitionLink href="/yazar" className="block outline-none">
                        <Button className="w-full bg-[#f9f9f9] hover:bg-white text-black font-black border-2 border-black shadow-[3px_3px_0px_0px_#000] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all rounded-[8px] h-11 uppercase text-sm group">
                    ...
                    <Button
                        ...
                        className="w-full mt-3 bg-black hover:bg-black/90 text-white font-black border-2 border-black shadow-[3px_3px_0px_0px_rgba(255,255,255,0.28)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all rounded-[8px] h-11 uppercase text-sm group"
                    >
            ...
            {/* Suggested Writers */}
            <div className="bg-[#222226] backdrop-blur-xl border-2 border-black rounded-[8px] p-5 shadow-[3px_3px_0_0_#000]">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                    <UserPlus className="w-5 h-5 text-[#23A9FA]" />
                    Takip Önerileri
                </h3>
                    ...
                    {suggestedUsers && suggestedUsers.length > 0 ? (
                        suggestedUsers.map((user) => (
                            <div key={user.id} className="flex items-center gap-3 justify-between group">
                                <Link href={`/kullanici/${user.username || user.id}`} className="flex items-center gap-3 overflow-hidden flex-1 relative z-10 outline-none">
                                    <OptimizedAvatar
                                        src={user.avatar_url}
                                        alt={user.username || ""}
                                        size={40}
                                    ...
                                    <div className="text-sm min-w-0 pr-2">
                                        <div className="font-bold truncate group-hover:underline text-foreground">{user.full_name || user.username || "Fizikhub üyesi"}</div>

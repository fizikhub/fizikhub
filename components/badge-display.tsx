...
export function BadgeDisplay({ userBadges, maxDisplay = 4, size = "md" }: BadgeDisplayProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(val) => {
            ...
            <DialogContent className="sm:max-w-4xl h-[85vh] p-0 flex flex-col gap-0 overflow-hidden bg-[#FAF9F6] border-[4px] border-black shadow-[12px_12px_0px_rgba(0,0,0,1)] rounded-none sm:rounded-2xl">
                ...
                {/* Header */}
                <DialogHeader className="p-6 bg-white border-b-[3px] border-black z-10 shrink-0">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            {selectedBadge ? (
                                <button
                                    onClick={() => setSelectedBadge(null)}
                                    className="p-2 -ml-2 rounded-lg hover:bg-black hover:text-white transition-colors border-2 border-transparent hover:border-black group"
                                >
                                    <ChevronRight className="w-8 h-8 rotate-180 stroke-[3px]" />
                                </button>
                            ) : (
                                <div className="p-2 bg-yellow-400 border-2 border-black rounded-lg shadow-[4px_4px_0px_#000]">
                                    <Trophy className="w-6 h-6 text-black stroke-[3px]" />
                                </div>
                            )}

                            <div>
                                <DialogTitle className="text-3xl font-black uppercase tracking-tighter text-black flex items-center gap-3 italic">
                                    {selectedBadge ? "Çıkartma Detayı" : "Koleksiyon Kitabı"}
                                </DialogTitle>
                                {!selectedBadge && (
                                    <p className="text-xs font-medium text-black/40 uppercase tracking-widest mt-1">
                    ...
                    <AnimatePresence mode="wait">
                        {selectedBadge ? (
                            ...
                            >
                                ...
                                <div className="w-full md:w-1/2 p-8 flex items-center justify-center bg-[#f0f0f0] border-b-[3px] md:border-b-0 md:border-r-[3px] border-black relative overflow-hidden">
                                    {/* Sticker Background Blob */}
                                    <motion.div
                                        className="absolute w-64 h-64 bg-yellow-400 rounded-full mix-blend-multiply opacity-20"
                                        animate={{ scale: [1, 1.2, 1], rotate: [0, 90, 0] }}
                                        transition={{ duration: 10, repeat: Infinity, ease: "linear" }}
                                    />
                                    <motion.div
                                        className="absolute w-64 h-64 bg-blue-400 rounded-full mix-blend-multiply opacity-20 left-10 top-10"
                                    ...
                                    >
                                        <CustomBadgeIcon name={selectedBadge.badges.name} className="w-full h-full drop-shadow-[10px_10px_0px_rgba(0,0,0,0.2)]" />
                                    </motion.div>
                                </div>
                                ...
                                <div className="w-full md:w-1/2 p-8 overflow-y-auto bg-white">
                                    <div className="space-y-8">
                                        <div>
                                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-black text-white border-2 border-black rounded-lg text-sm font-bold uppercase tracking-wider mb-4 shadow-[4px_4px_0px_#94a3b8]">
                                                <Star className="w-4 h-4 fill-current stroke-none" />
                                                {selectedBadge.badges.category || "Genel"}
                                            </div>
                                            <h2 className="text-4xl md:text-5xl font-black uppercase leading-[0.9] text-black mb-2 tracking-tighter">
                                                {selectedBadge.badges.name}
                                            </h2>
                                            <div className="text-sm font-bold text-black/40 font-mono mt-2 bg-black/5 inline-block px-2 py-1 rounded">
                                                KAZANILDI: {new Date(selectedBadge.awarded_at).toLocaleDateString('tr-TR', { day: 'numeric', month: 'long', year: 'numeric' })}
                                            </div>
                                        </div>

                                        <div className="space-y-6">
                                            <div className="p-6 bg-blue-50 border-[3px] border-black rounded-xl shadow-[6px_6px_0px_#000]">
                                                <h3 className="text-sm font-black text-blue-600 uppercase mb-2 tracking-widest">AÇIKLAMA</h3>
                                            ...
                                            <div className="p-6 bg-yellow-50 border-[3px] border-black rounded-xl shadow-[6px_6px_0px_#000] relative overflow-hidden">
                                                <div className="absolute top-0 right-0 p-3 opacity-10">
                                                    <Trophy className="w-32 h-32 rotate-12 stroke-[1px]" />
                                                </div>
                                                <h3 className="text-sm font-black text-yellow-600 uppercase mb-2 tracking-widest relative z-10">NASIL KAZANILIR?</h3>
                                                <p className="text-lg font-bold text-black relative z-10">
                                                    {/* Simulated criteria */}
                                ...
                                >
                                    {categories.map((category, catIndex) => (
                                        <div key={category} className="space-y-6">
                                            <div className="flex items-center gap-4">
                                                <h3 className="text-2xl font-black uppercase tracking-tighter text-black italic bg-white border-2 border-black px-4 py-1 shadow-[4px_4px_0px_#000] -rotate-1 inline-block transform origin-left">
                                                    {category}
                                                </h3>
                                            ...
                                            <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-6">
                                                {badgesByCategory[category].map((badge, idx) => (
                                                    <motion.button
                                                        ...
                                                        key={badge.badges.id}
                                                        className="group relative flex flex-col items-center gap-3 p-4 rounded-xl bg-white border-2 border-black hover:border-black hover:shadow-[8px_8px_0px_#000] transition-all cursor-pointer overflow-visible"
                                                    >
                                                        <div className="absolute -top-3 -right-3 w-8 h-8 bg-black text-white rounded-full flex items-center justify-center font-black text-xs border-2 border-white z-20 shadow-md opacity-0 group-hover:opacity-100 transition-opacity scale-0 group-hover:scale-100">
                                                            GO
                                                        </div>

                                                        <div className="relative w-24 h-24 drop-shadow-md z-10 filter group-hover:drop-shadow-none transition-all">
                                                        ...
                                                        <div className="text-center w-full z-10 pt-2 border-t-2 border-black/5 w-full">
                                                            <div className="font-black text-sm text-black uppercase truncate px-1">
                                                                {badge.badges.name}
                                                            </div>
                                                            <div className="text-[10px] text-black/40 font-mono mt-1 font-bold">
                                                ...
                                                {Array.from({ length: Math.max(0, 4 - badgesByCategory[category].length) }).map((_, i) => (
                                                    <div
                                                        ...
                                                        className="flex flex-col items-center justify-center gap-3 p-4 rounded-xl border-2 border-dashed border-black/20 bg-black/[0.02] opacity-60 grayscale"
                                                    >
                                                        <div className="w-20 h-20 rounded-full bg-black/5 flex items-center justify-center border-2 border-black/10">

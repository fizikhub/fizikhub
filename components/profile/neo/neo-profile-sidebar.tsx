        ...
        <div className="space-y-6 lg:sticky lg:top-32 transition-all duration-300">
            ...
            <div className="bg-[#09090b] border-2 border-black rounded-xl p-5 shadow-[4px_4px_0px_#000]">
                <h3 className="font-black text-white mb-4 flex items-center gap-2 text-sm tracking-widest uppercase font-[family-name:var(--font-outfit)]">
                    <div className="w-1.5 h-4 bg-[#FACC15] skew-x-[-12deg]" />
                    KÜNYE
                </h3>

                <div className="space-y-4">
                    <div className="flex flex-col gap-3 text-xs font-bold text-zinc-400">
                        <div className="flex items-center gap-3 p-3 bg-zinc-900/50 rounded-lg border-2 border-zinc-800 hover:border-[#FACC15] transition-colors group">
                            <Calendar className="w-4 h-4 text-zinc-500 group-hover:text-[#FACC15] transition-colors" />
                            <span>Katılım: <span className="text-white ml-1">{format(new Date(user?.created_at || Date.now()), 'MMMM yyyy', { locale: tr })}</span></span>
                        </div>
                        {profile?.website && (
                            <a href={profile.website} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-zinc-900/50 rounded-lg border-2 border-zinc-800 hover:border-[#FACC15] hover:bg-black transition-all group active:translate-y-0.5">
                                <LinkIcon className="w-4 h-4 text-zinc-500 group-hover:text-[#FACC15] transition-colors" />
                                <span className="truncate max-w-[200px] text-zinc-300 group-hover:text-white group-hover:underline decoration-[#FACC15] decoration-2 underline-offset-2">{profile.website.replace(/^https?:\/\//, '')}</span>
                            </a>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-[#09090b] border-2 border-black rounded-xl p-5 shadow-[4px_4px_0px_#000]">
                    <h3 className="font-black text-white mb-4 flex items-center justify-between text-sm tracking-wide font-[family-name:var(--font-outfit)]">
                        <span className="flex items-center gap-2 uppercase tracking-widest">
                            <Award className="w-5 h-5 text-black fill-[#FACC15]" />
                            ROZETLER
                        </span>
                        <span className="text-[10px] bg-[#FACC15] text-black font-black px-2 py-0.5 rounded border border-black shadow-[2px_2px_0px_#000]">
                            {userBadges.length}
                        </span>
                    </h3>
                        ...
                        {userBadges.map((badgeItem: any, i: number) => {
                            ...
                            return (
                                <div key={i} className="group relative aspect-square">
                                    <div className="w-full h-full bg-zinc-900 border-2 border-zinc-800 rounded-lg flex items-center justify-center text-xl text-zinc-400 hover:text-[#FACC15] hover:border-[#FACC15] hover:bg-black transition-all cursor-help shadow-sm hover:shadow-[3px_3px_0px_#FACC15] hover:-translate-y-1">
                                        {badge.icon || "🏆"}
                                    </div>
                                    {/* Tooltip */}
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-[180px] bg-black border-2 border-[#FACC15] text-white text-xs p-3 rounded-none shadow-[4px_4px_0px_#000] opacity-0 group-hover:opacity-100 pointer-events-none transition-all z-50 translate-y-2 group-hover:translate-y-0">
                                        <div className="font-black mb-1 text-[#FACC15] uppercase tracking-wide text-[11px] border-b border-zinc-800 pb-1">{badge.name}</div>
                                        <div className="text-zinc-300 text-[11px] leading-tight font-medium pt-1">{badge.description}</div>
                                        {/* Little Triangle */}
                                        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-black border-r-2 border-b-2 border-[#FACC15] rotate-45 transform"></div>
                                    </div>
                                </div>
                            );
                        })}

...
-- CREATE INDEX CONCURRENTLY keeps writes available while indexes are being built.
-- =====================================================================================
...
-- -------------------------------------------------------------------------------------
create index concurrently if not exists idx_fh_articles_published_created_id
    on public.articles (created_at desc, id desc)
    where status = 'published';

create index concurrently if not exists idx_fh_articles_published_category_created
    on public.articles (category, created_at desc, id desc)
    where status = 'published' and category is not null;

create index concurrently if not exists idx_fh_articles_published_slug
    on public.articles (slug)
    where status = 'published';

create index concurrently if not exists idx_fh_articles_author_status_created
    on public.articles (author_id, status, created_at desc);

create index concurrently if not exists idx_fh_articles_title_trgm_published
    on public.articles using gin (title gin_trgm_ops)
    where status = 'published';

create index concurrently if not exists idx_fh_articles_excerpt_trgm_published
    on public.articles using gin (excerpt gin_trgm_ops)
    where status = 'published' and excerpt is not null;

-- Article detail side data.
create index concurrently if not exists idx_fh_article_likes_article_user
    on public.article_likes (article_id, user_id);

create index concurrently if not exists idx_fh_article_bookmarks_article_user
    on public.article_bookmarks (article_id, user_id);

create index concurrently if not exists idx_fh_article_comments_article_created_user
    on public.article_comments (article_id, created_at asc, user_id);

create index concurrently if not exists idx_fh_article_references_article_created
    on public.article_references (article_id, created_at asc);

-- -------------------------------------------------------------------------------------
-- Forum: /forum listing, detail page, vote/like/comment side queries.
-- -------------------------------------------------------------------------------------
create index concurrently if not exists idx_fh_questions_created_id
    on public.questions (created_at desc, id desc);

create index concurrently if not exists idx_fh_questions_votes_created
    on public.questions (votes desc, created_at desc, id desc);

create index concurrently if not exists idx_fh_questions_category_created
    on public.questions (category, created_at desc, id desc)
    where category is not null;

create index concurrently if not exists idx_fh_questions_author_created
    on public.questions (author_id, created_at desc);

create index concurrently if not exists idx_fh_questions_title_trgm
    on public.questions using gin (title gin_trgm_ops);

create index concurrently if not exists idx_fh_answers_question_created
    on public.answers (question_id, created_at asc, id asc);

create index concurrently if not exists idx_fh_answers_author_created
    on public.answers (author_id, created_at desc);

create index concurrently if not exists idx_fh_answer_likes_answer_user
    on public.answer_likes (answer_id, user_id);

create index concurrently if not exists idx_fh_answer_comments_answer_created_user
    on public.answer_comments (answer_id, created_at asc, author_id);

create index concurrently if not exists idx_fh_answer_comment_likes_comment_user
    on public.answer_comment_likes (comment_id, user_id);

create index concurrently if not exists idx_fh_question_votes_question_user
    on public.question_votes (question_id, user_id);

create index concurrently if not exists idx_fh_question_bookmarks_question_user
    on public.question_bookmarks (question_id, user_id);

-- -------------------------------------------------------------------------------------
-- Profiles, stories, dictionary, quizzes, notifications.
-- -------------------------------------------------------------------------------------
create index concurrently if not exists idx_fh_profiles_writer_reputation
    on public.profiles (reputation desc, id)
    where is_writer = true;

create index concurrently if not exists idx_fh_profiles_username_not_null
    on public.profiles (username)
    where username is not null;

create index concurrently if not exists idx_fh_stories_active_created
    on public.stories (expires_at, created_at desc)
    where expires_at is not null;

create index concurrently if not exists idx_fh_story_groups_created
    on public.story_groups (created_at desc, id desc);

create index concurrently if not exists idx_fh_dictionary_terms_term
    on public.dictionary_terms (term asc);

create index concurrently if not exists idx_fh_quizzes_created
    on public.quizzes (created_at desc, id desc);

create index concurrently if not exists idx_fh_quiz_questions_quiz_order
    on public.quiz_questions (quiz_id, "order" asc);

create index concurrently if not exists idx_fh_notifications_recipient_read_created

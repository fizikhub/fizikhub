...
export function AuthorPanelClient({ initialArticles }: { initialArticles: any[] }) {
                ...
                setArticles(prev => prev.map(art => {
                    if (art.id === articleId) {
                        return { 
                            ...
                            hasApproved: true,
                            // Optimizasyon için mock approver eklenebilir, fakat tam senkronizasyon için sayfayı yenilemek veya actiondan objeyi dönmek daha sağlıklı olur
                        };
                    }
                    return art;
                }));
    ...
    const handleRevoke = async (articleId: number) => {
        setLoadingMap(prev => ({ ...prev, [articleId]: true }));
        try {
        ...
        <TooltipProvider>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {articles.map((article) => {
                    ...
                    const dateRaw = new Date(article.created_at);
                    const isApproved = article.hasApproved;
                    const isLoading = loadingMap[article.id];
                    const count = article.approvalCount || 0;
                    const approvers = article.approvers || [];
                    
                    return (
                        <div key={article.id} className="group relative bg-background border-2 border-black dark:border-zinc-800 rounded-xl p-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] dark:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.5)] flex flex-col transition-all hover:-translate-y-1 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] dark:hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,0.5)]">
                            ...
                            <div className="flex items-center justify-between mb-4 pb-4 border-b-2 border-dashed border-zinc-800">
                                <div className="flex items-center gap-2">
                                    <div className="w-8 h-8 rounded-full border border-zinc-700 bg-zinc-900 flex items-center justify-center overflow-hidden">
                                        {author?.avatar_url ? (
                                            <img src={author.avatar_url} alt={author.full_name || "Yazar"} className="w-full h-full object-cover" />
                                        ) : (
                                ...
                                <div className="flex items-center gap-1.5 text-xs font-bold text-zinc-400 bg-zinc-900 px-2.5 py-1 rounded-md border border-zinc-800">
                                    ...
                                    <span>{format(dateRaw, "d MMM", { locale: tr })}</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-auto">
                                ...
                                <div className="mb-4">
                                    ...
                                    <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden border border-zinc-800">
                                        <div className="h-full bg-primary transition-all duration-500 ease-out" style={{ width: `${Math.min(100, (count / REQUIRED_APPROVALS) * 100)}%` }} />
                                    </div>
                                </div>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    {isApproved ? (
                                        <Button 
                                            onClick={() => handleRevoke(article.id)}
                                            disabled={isLoading}
                                            variant="destructive"
                                        ...
                                        >
                                            {isLoading ? <span className="animate-pulse">İşleniyor...</span> : "Onayla"}
                                        </Button>
                                    )}
                                </div>
                            </div>

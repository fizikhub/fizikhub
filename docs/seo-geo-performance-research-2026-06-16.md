...
## 2026-06-23 CTR, GEO manifesti ve telemetry dayanıklılığı

- Yeni GSC exportu bulunmadığı için 9 Mayıs verisi yalnız mevcut fırsatlarda kullanıldı. İlk sayfada veya sınırında olup düşük TO üreten siyah cisim (%0,38; ortalama pozisyon 7,68) ve periyodik hareket (%0,47; 9,70) başlıklarından yıl/rehber/CTA kalabalığı çıkarıldı. Yeni başlıklar sorguyla birebir, site adı eklendiğinde yaklaşık 55 karakterlik SERP uzunluğundadır.
- `ai-index.json` öncelikli makaleler için artık sayfadaki gerçek şema grafiğini bildirir: Article, BlogPosting, WebPage, BreadcrumbList, LearningResource ve intent içeriğinde FAQPage ile DefinedTerm.
- Next.js Web Vitals attribution alanının production örneklerinde boş kalması üzerine native LargestContentfulPaint `PerformanceObserver` yedeği eklendi. Böylece element, resource URL, render/load zamanı ve boyut sonraki örneklerde framework alanından bağımsız yakalanabilir.
- Supabase CSP/LCP migrationlarının production'da henüz uygulanmadığı doğrulandı. CSP report-only ihlalleri bu nedenle veritabanına ek olarak fingerprint ile deduplike edilmiş structured Vercel loglarına yazılır; telemetry tablo yokken sessizce kaybolmaz.
